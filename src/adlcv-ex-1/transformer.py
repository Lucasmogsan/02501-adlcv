...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        #print('keys shape init: ', keys.shape)
        # First split the embed_dim to num_heads x head_dim
        keys = rearrange(keys, 'b seq_len (num_heads head_dim) -> b seq_len num_heads head_dim', num_heads=self.num_heads, head_dim=self.head_dim) # B x seq_len x num_heads x head_dim
        #print('keys shape split: ', keys.shape)
        # Secondly merge the batch_size with the num_heads
        keys = rearrange(keys, 'b seq_len num_heads head_dim -> (b num_heads) seq_len head_dim') # (B x num_heads) x seq_len x head_dim
        #print('keys shape merge: ', keys.shape)
        #print('keys shape transpose: ', keys.transpose(-2, -1).shape)
        
        # HINT repeat the same process for queries and values
        queries = rearrange(queries, 'b seq_len (num_heads head_dim) -> b seq_len num_heads head_dim', num_heads=self.num_heads, head_dim=self.head_dim) # B x seq_len x num_heads x head_dim
        queries = rearrange(queries, 'b seq_len num_heads head_dim -> (b num_heads) seq_len head_dim') # (B x num_heads) x seq_len x head_dim

        values = rearrange(values, 'b seq_len (num_heads head_dim) -> b seq_len num_heads head_dim', num_heads=self.num_heads, head_dim=self.head_dim) # B x seq_len x num_heads x head_dim
        values = rearrange(values, 'b seq_len num_heads head_dim -> (b num_heads) seq_len head_dim') # (B x num_heads) x seq_len x head_dim

        # Compute attetion logits
        # multiply queries and keys
        attention_logits = queries @ keys.transpose(-2, -1) # (B x num_heads) x seq_len x seq_len
        #print('attention_logits shape: ', attention_logits.shape)
        attention_logits = attention_logits * self.scale
        # softmax on attention
        attention = F.softmax(attention_logits, dim=-1) # (B x num_heads) x seq_len x seq_len
        #print('attention shape: ', attention.shape)
        #print('avg value:', attention.mean(dim=-1))
        # multiply attention with values
        out = attention @ values # (B x num_heads) x seq_len x head_dim
...
class TransformerClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        #print('tokens shape befpre cls: ', tokens.shape)

        if self.pool == 'cls':
            # Include cls token in the input sequence (repeat the cls token of the batch dimension)
            tokens = torch.cat((self.cls_token.expand(batch_size, -1, -1), tokens), dim=1)

        #print('tokens shape after cls: ', tokens.shape)
        ...
        if self.pool =='max':
            ...
            x = x.mean(dim=1)
            #print('x shape after mean pool: ', x.shape)

        elif self.pool == 'cls':
            # Get cls token (the first output token of the transfomer)
            x = x[:, 0]
            #print('x shape after cls pool: ', x.shape)
